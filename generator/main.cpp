...
void cone(float radius, float height, int slices, int stacks, string filename) {
    ...
    float deltah = (2.0f * (float) M_PI / (float) slices),
          deltav = height / (float) stacks,
          deltaRaio = radius / (float) stacks;
    float y = height / 2;
    ...
    for(int j = 0; j < stacks; j++) {
        for(int i = 0; i < slices; i++) {
            Patch patch;
            float angle1 = (float) i * deltah,
                  angle2 = (float) (i + 1) * deltah;
            if(j == 0) {
                // Base
...
void sphere(float radius, int slices, int stacks, string filename){
    Model model;
    float deltah = (2.0f * (float) M_PI/ (float) slices),
          deltav = (float) M_PI/ (float) stacks;
    for(int i = 0; i < slices; i++) {
        for(int j = 0; j < stacks; j++) {
            Patch patch;
            float beta1 = (float) i * deltah,
                  beta2 = (float) (i + 1) * deltah;
            float theta1 = (float) j * deltav,
                  theta2 = (float) (j + 1) * deltav;
            if (j == 0) {
                // Triangulo de cima
...
void tronco(float radius1, float radius2, float height, int slices, int stacks, string filename){
    ...
    float deltah = (2.0f * (float) M_PI / (float) slices),
          deltav = height / (float) stacks,
          deltaRaio = abs(radius1 - radius2) / (float) stacks;
    float y = height / 2;
    float alturaS, alturaS2;
    for(int j = 0; j < stacks; j++){
        for(int i = 0; i < slices; i++){
